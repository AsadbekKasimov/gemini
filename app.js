// ===== CONFIGURATION =====
// Вставьте сюда ссылку на ваш Google Script (Web App URL)
const GOOGLE_SCRIPT_CHECK_URL = "https://script.google.com/macros/s/AKfycbwKVDL8LJBf1tXfAwWfsqmqC6h8b22YOSfMCELM0x0D59ra_RdJRaHlPgKzQZI4rEo-xg/exec"; 
...
async function checkUserAccess() {
    ...
    // Если открыто не из Telegram или нет пользователя (например, в браузере)
    if (!user || !user.id) {
        // Для тестов в браузере можно раскомментировать строку ниже:
        // return true; 
        showBlockedScreen("Запуск разрешен только через Telegram");
        return false;
    }

    try {
        // Отправляем запрос к Google Script
        const response = await fetch(`${GOOGLE_SCRIPT_CHECK_URL}?userId=${user.id}`);
        ...
        // В случае ошибки сети можно либо пустить, либо заблокировать. 
        // Лучше показать ошибку.
        document.getElementById('access-text').textContent = "Ошибка соединения. Попробуйте позже.";
        return false;
    }
}

function showBlockedScreen(message = null) {
    document.getElementById('access-screen').classList.add('hidden');
    const blockedScreen = document.getElementById('blocked-screen');
    blockedScreen.classList.remove('hidden');
    if (message) {
        blockedScreen.querySelector('p').textContent = message;
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    // 1. Сначала проверяем доступ
    const hasAccess = await checkUserAccess();
    
    if (hasAccess) {
        // 2. Если доступ есть — запускаем остальное приложение
        appInitialized = true;
        loadProducts();
        setupEventListeners();
        updateCartBadge();
